...
export async function generateSVG(
  ...
  const bricksByClass: Record<string, { brick: Brick; index: number }[]> = {};
  bricks.forEach((brick, i) => {
    if (!bricksByClass[brick.colorClass]) bricksByClass[brick.colorClass] = [];
    ...
    .map(([colorClass, brickGroup]) => {
      const uses = brickGroup
        .map(({ brick, index }) => {
          ...
          if (!anim.animate) return useElem;
