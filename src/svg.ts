...
function simulate(
  ...
  while (
    ...
    paddlePositionX = Math.max(
      padding,
      Math.min(canvasWidth - padding - paddleWidth, ballX - paddleWidth / 2),
    );
    ...
    if (
      ballX + ballVelocityX > canvasWidth - padding - ballRadius ||
    ...
    ballX = Math.max(
      padding + ballRadius,
      Math.min(canvasWidth - padding - ballRadius, ballX),
    );
    ballY = Math.max(
      padding + ballRadius,
      Math.min(canvasHeight - padding - ballRadius, ballY),
    );
...
export async function generateSVG(
  ...
  const canvasWidth =
    brickColumnCount * (brickSize + brickGap) + padding * 2 - brickGap; // right edge flush
