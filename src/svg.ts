...
function simulate(
  ...
  while (
    ...
    paddlePositionX = Math.max(
      0,
      Math.min(canvasWidth - paddleWidth, ballX - paddleWidth / 2),
    );
    ...
    if (
      ballX + ballVelocityX > canvasWidth - ballRadius ||
...
export async function generateSVG(
  ...
  const canvasWidth =
    brickColumnCount * (brickSize + brickGap) + brickOffsetLeft * 2 - brickGap; // right edge flush
